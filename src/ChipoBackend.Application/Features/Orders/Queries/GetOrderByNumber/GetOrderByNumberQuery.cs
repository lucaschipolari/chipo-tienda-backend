...
namespace ChipoBackend.Application.Features.Orders.Queries.GetOrderByNumber;
...
) : IRequestHandler<GetOrderByNumberQuery, OrderDto>
{
    ...
    {
        var order = await orderRepository.GetByOrderNumberAsync(request.OrderNumber, ct)
            ?? throw new NotFoundException($"Pedido '{request.OrderNumber}' no encontrado.");

        // For registered customers, load full name and email
